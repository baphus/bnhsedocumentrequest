...
class RequestsTable extends DataTableComponent
{
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make('Document')
                ->label(fn($row) => <<<HTML
                    <div class="flex flex-col">
                        ...
                        <span class="text-xs text-gray-500 font-semibold text-bnhs-blue">Qty: {$row->quantity}</span>
                    </div>
                HTML)
                ->html(),
            ...
            LinkColumn::make('Actions')
                ...
                ->location(fn($row) => route('admin.requests.show', $row->id))
                ->attributes(fn($row) => [
    ...
    public function setStatusPending(): void { $this->bulkUpdateStatus('pending'); }
    public function setStatusProcessing(): void { $this->bulkUpdateStatus('processing'); }
    public function setStatusReady(): void { $this->bulkUpdateStatus('ready'); }
    public function setStatusCompleted(): void { $this->bulkUpdateStatus('completed'); }
