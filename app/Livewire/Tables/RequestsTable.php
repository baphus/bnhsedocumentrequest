...
class RequestsTable extends DataTableComponent
{
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make('Student Details', 'last_name')
    ...
    public function filters(): array
    {
        return [
            ...
            SelectFilter::make('Document Type')
                ...
                ->filter(function (Builder $query, string $value) {
                    if ($value !== '') {
                        $query->where('document_type_id', $value);
                    }
                }),
            ...
            SelectFilter::make('Track/Strand')
                ...
                ->filter(function (Builder $query, string $value) {
                    if ($value !== '') {
                        $query->where('track_strand', $value);
                    }
                }),
    ...
    protected function bulkUpdateStatus(string $status): void
    {
        ...
        // Force IDs to be integers for the query
        $ids = array_map(fn($id) => (int) $id, $ids);

        // Wrap in transaction for better data integrity
        DB::transaction(function () use ($ids, $status) {
            // Chunk the update for better performance and to handle large selections
            Request::whereIn('id', $ids)->chunk(100, function ($requests) use ($status) {
